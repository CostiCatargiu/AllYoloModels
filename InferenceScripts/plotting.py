...
class Annotator:
    ...
    def box_label(self, box, label="", color=(128, 128, 128), txt_color=(255, 255, 255), rotated=False):
        ...
        if len(label) > 1:
            my_list = label.split()
            my_list = [' '.join(my_list[:-1]), my_list[-1]]
        else:
            my_list = label.split()
        with open("/home/constantin/Doctorat/YoloModels/YoloLib2/InferenceScripts/labels_list.txt", 'a') as f:
            f.write(label + ",")
        for i in range(0, len(classesFilter)):
            if classesFilter[i] != my_list[0]:
