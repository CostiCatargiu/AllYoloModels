...
classes_names = os.environ.get('NAMES_PARAMETER')
classes_names = classes_names.strip('[]').split(', ')
...
class Evaler:
    def __init__(self,
        ...
        self.height = height
        self.width = width
        self.ncc=int(os.environ.get('NC_PARAMETER'))
    ...
    def predict_model(self, model, dataloader, task):
        ...
        if self.do_pr_metric:
            ...
            if self.plot_confusion_matrix:
                ...
                confusion_matrix = ConfusionMatrix(nc=self.ncc)
        ...
        if self.do_pr_metric:
            ...
            if len(stats) and stats[0].any():
                ...
                p, r, ap, f1, ap_class = ap_per_class(*stats, plot=self.plot_curve, save_dir=self.save_dir, names=self.names)
                ...
                nt = np.bincount(stats[3].astype(np.int64), minlength=self.ncc)  # number of targets per class
                ...
                if self.verbose and self.ncc > 1:
                    for i, c in enumerate(ap_class):
                        LOGGER.info(pf % (self.names[c], seen, nt[c], p[i, AP50_F1_max_idx], r[i, AP50_F1_max_idx],
                                           f1[i, AP50_F1_max_idx], ap50[i], ap[i]))

                if self.plot_confusion_matrix:
                    confusion_matrix.plot(save_dir=self.save_dir, names=list(self.names))
            else:
    ...
    def eval_model(self, pred_results, model, dataloader, task):
        ...
        if task != 'speed' and len(pred_results):
            ...
            if self.verbose:
                ...
                label_count_dicts = [copy.deepcopy(label_count_dict) for _ in range(self.ncc)]
                ...
                for nc_i in range(self.ncc):
                    ...
                    f1 = 2 * p * r / (p + r + 1e-16)
                    i = f1.argmax()
                    LOGGER.info(pf % (self.names[nc_i], len(label_count_dicts[nc_i]["images"]), label_count_dicts[nc_i]["anns"], p[i], r[i], f1[i], map50, map))
            cocoEval.summarize()
