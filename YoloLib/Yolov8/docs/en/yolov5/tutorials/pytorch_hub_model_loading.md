...
```python
model = torch.hub.load('ultralytics/yolov5', 'custom', path='path/to/best_80epochs.pt')  # local model
model = torch.hub.load('path/to/yolov5', 'custom', path='path/to/best_80epochs.pt', source='local')  # local repo
```
