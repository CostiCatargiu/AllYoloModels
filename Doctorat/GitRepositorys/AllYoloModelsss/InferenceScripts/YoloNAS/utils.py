...
def draw_text_box(
    ...
    text_left_offset = 7
    if y < 2:
        image = cv2.rectangle(image, (x, y), (x + text_width + text_left_offset, y - text_height - int(15 * font_size)+60), background_color, -1)
        image = cv2.putText(image, text, (x + text_left_offset, y - int(10 * font_size)+30), font, font_size, text_color, thickness, lineType=cv2.LINE_AA)
    else:
